...
<head>
    ...
    <style>
        .list .list-left,.list-right,.bottom,.page{
            float:left;
        }
        .list-left{
        ...
        .list-right{
            width:800px;border: 1px solid #C7C0C0;margin-left: 20px;margin-top:30px;display: inline-block;height:auto;padding-left: 10px;
        }
        .left-inp{
        ...
        .article-title{
            width:700px;font-size:23px;margin-left: 25px;margin-top: 25px;padding-right:25px;
            color: black;
        }
        .article-info{
            float: left;margin-left: 25px;margin-top: 18px;color: #0abbb9;
        }
        .list-num{
        ...
        .article-contents{
            margin-left: 25px;margin-top: 18px;width:700px;
            padding-right: 25px;
        }
        .article-desc{
        ...
        .article-content{
            margin-top: 18px;text-indent:2em;margin-bottom: 25px;
        }
        .page-up,.page-down{
            float: right;width:390px;border: 1px solid #C7C0C0;margin-top: 20px;margin-left: 10px;text-align: center;
        }
        .page{
            height:100px;margin-left: 365px;
        }
    </style>
    <!--跳转代码-->
...
<div id="container">
    ...
    <div class="list">
        ...
        <div class="list-right" id="list-right">
            <div class="article-title">
                {{$article->title}}
            </div>
            <div>
                <ul>
                    <li class="article-info">{{$article->create_at}}</li>
                    <li class="article-info">作者：{{$article->author_name}}</li>
                    <li class="article-info list-show"><img src="{{$path}}setting/show.png" alt=""> <div class="list-num">{{$article->showNum}}</div></li>
                </ul>
            </div>
        ...
        <div class="page">
            @if($article->pageupid != 0)
            <a href="/group=news&action=index&method=articledetail?id={{$article->pageupid}}">
                <div class="page-up"> 下一条: {{$article->pageuptitle}}</div>
            </a>
            @else
                <div class="page-up">{{$article->pageuptitle}}</div>
            @endif

            @if($article->pagedownid != 0)
            <a href="/group=news&action=index&method=articledetail?id={{$article->pagedownid}}">
                <div  class="page-down">上一条:  {{$article->pagedowntitle}}</div>
            </a>
            @else
                <div  class="page-down">{{$article->pagedowntitle}}</div>
            @endif
        </div>
    ...
    <div class="bottom">
        <div id="footer">
            <div style=" height:30px; line-height:30px; padding-top:10px; text-align:center; font-size:13px;">
                <a href="/" class="bottom-a">首页</a>&nbsp;|&nbsp;
                <a href="" class="bottom-a">走进鹊经堂</a> &nbsp;|&nbsp;
                <a href="" class="bottom-a">新闻中心</a> &nbsp;|&nbsp;
                <a href="" class="bottom-a">主营业务</a> &nbsp;|&nbsp;
                <a href="" class="bottom-a">商务服务</a> &nbsp;|&nbsp;
                <a href="" class="bottom-a">企业文化</a> &nbsp;|&nbsp;
                <a href="" class="bottom-a">人才战略</a> &nbsp;|
                <a href="" class="bottom-a">服务交流</a>
            </div>

            <div style=" height:30px; line-height:30px;  text-align:left;">
                <span style="color:#F00;">友情链接：</span>
                <a href="/" target="_blank" class="bottom-a">点击申请友情链接</a>&nbsp;
...
<script>
    ...
    var contentHeight2 = window.getComputedStyle(box2).height;
    if(parseInt(contentHeight) < parseInt(contentHeight2)){
        console.log(contentHeight);
        box.style.height=contentHeight2;
    }
